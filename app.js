...
import { ADJECTIVES, NOUNS, USER_COLORS } from './src/app-constants.js';
import { createDocumentStore } from './src/document-store.js';
import { createCollaborationController } from './src/collaboration-room.js';
...
function disconnectCollaboration() {
    return collaborationController.disconnectCollaboration();
}
...
function buildControllerDeps() {
    return {
        ...
        setSupabase: value => { supabase = value; },
        getCurrentUser: () => currentUser,
        setCurrentUser: value => { currentUser = value; },
        getPendingAuthEmail: () => pendingAuthEmail,
        setPendingAuthEmail: value => { pendingAuthEmail = value; },
        getSupabaseDocumentId: () => supabaseDocumentId,
        setSupabaseDocumentId: value => { supabaseDocumentId = value; },
        getSupabaseSaveTimer: () => supabaseSaveTimer,
        setSupabaseSaveTimer: value => { supabaseSaveTimer = value; },
        getIsSupabaseSaving: () => isSupabaseSaving,
        setIsSupabaseSaving: value => { isSupabaseSaving = value; },
        getHasCollaborationRpc: () => hasCollaborationRpc,
        setHasCollaborationRpc: value => { hasCollaborationRpc = value; },
        getRoomWasAutoGenerated: () => roomWasAutoGenerated,
        setRoomWasAutoGenerated: value => { roomWasAutoGenerated = value; },
        getActiveRoomReadyPromise: () => activeRoomReadyPromise,
        setActiveRoomReadyPromise: value => { activeRoomReadyPromise = value; },
        getResolveActiveRoomReady: () => resolveActiveRoomReady,
        setResolveActiveRoomReady: value => { resolveActiveRoomReady = value; },
        getRoomConnectionGeneration: () => roomConnectionGeneration,
        bumpRoomConnectionGeneration: () => (++roomConnectionGeneration),
        getHasBootstrappedInitialRoom: () => hasBootstrappedInitialRoom,
        setHasBootstrappedInitialRoom: value => { hasBootstrappedInitialRoom = value; },
        getActiveAuthBootstrapPromise: () => activeAuthBootstrapPromise,
        setActiveAuthBootstrapPromise: value => { activeAuthBootstrapPromise = value; },
        getRequiresSupabaseHydration: () => requiresSupabaseHydration,
        setRequiresSupabaseHydration: value => { requiresSupabaseHydration = value; },
        getYDoc: () => ydoc,
        setYDoc: value => { ydoc = value; },
        getProvider: () => provider,
        setProvider: value => { provider = value; },
        getSharedMap: () => sharedMap,
        setSharedMap: value => { sharedMap = value; },
        getCheckboxMap: () => checkboxMap,
        setCheckboxMap: value => { checkboxMap = value; },
        getLessonsArray: () => lessonsArray,
        setLessonsArray: value => { lessonsArray = value; },
        getRolesArray: () => rolesArray,
        setRolesArray: value => { rolesArray = value; },
        getSharedHistoryArray: () => sharedHistoryArray,
        setSharedHistoryArray: value => { sharedHistoryArray = value; },
        getCurrentRoomId: () => currentRoomId,
        setCurrentRoomId: value => { currentRoomId = value; },
        getLocalNickname: () => localNickname,
        getLocalColor: () => localColor,
        connectToRoom: roomName => collaborationController.connectToRoom(roomName),
        disconnectCollaboration: () => collaborationController.disconnectCollaboration(),
        resolveSupabaseDocumentByRoom: roomId => documentStore.resolveSupabaseDocumentByRoom(roomId),
        findLatestAccessibleDocument: () => documentStore.findLatestAccessibleDocument(),
        findAccessibleDocumentByRoom: roomId => documentStore.findAccessibleDocumentByRoom(roomId),
        maybeSwitchToLatestDocument: () => documentStore.maybeSwitchToLatestDocument(),
        forceSwitchToLatestDocument: () => documentStore.forceSwitchToLatestDocument(),
        refreshCurrentRoomFromSupabase: () => documentStore.refreshCurrentRoomFromSupabase(),
        saveCurrentDocumentToSupabase: options => documentStore.saveCurrentDocumentToSupabase(options)
    };
}

function initializeControllers() {
    const deps = buildControllerDeps();
    documentStore = createDocumentStore(deps);
    collaborationController = createCollaborationController(deps);
    authController = createAuthController(deps);
}

async function bootstrapAuthenticatedApp() {
    return authController.bootstrapAuthenticatedApp();
}
...
async function bootstrapSupabaseDocumentForRoom() {
    return documentStore.bootstrapSupabaseDocumentForRoom();
}

async function findLatestAccessibleDocument() {
    return documentStore.findLatestAccessibleDocument();
}

async function findAccessibleDocumentByRoom(roomId) {
    return documentStore.findAccessibleDocumentByRoom(roomId);
}

async function resolveSupabaseDocumentByRoom(roomId) {
    return documentStore.resolveSupabaseDocumentByRoom(roomId);
}

async function maybeSwitchToLatestDocument() {
    return documentStore.maybeSwitchToLatestDocument();
}

async function forceSwitchToLatestDocument() {
    return documentStore.forceSwitchToLatestDocument();
}

async function refreshCurrentRoomFromSupabase() {
    return documentStore.refreshCurrentRoomFromSupabase();
}

async function resolveSupabaseDocumentForRoom() {
    return documentStore.resolveSupabaseDocumentForRoom();
}

function scheduleSupabaseAutoSave() {
    return documentStore.scheduleSupabaseAutoSave();
}

async function saveCurrentDocumentToSupabase(options = {}) {
    return documentStore.saveCurrentDocumentToSupabase(options);
}

async function sendOtpCode() {
    return authController.sendOtpCode();
}

async function verifyOtpCode() {
    return authController.verifyOtpCode();
}

async function logoutSupabase() {
    return authController.logoutSupabase();
}

function initSupabaseAuth() {
    return authController.initSupabaseAuth();
}

async function determineInitialRoom() {
    return authController.determineInitialRoom();
}
...
async function connectToRoom(roomName) {
    return collaborationController.connectToRoom(roomName);
}
